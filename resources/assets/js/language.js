    ...
    var table = $('.yajra-datatable').DataTable({
        ...
        columns: [
            ...
            render: function(data, type, row) {
                let editButton = '<a href="/admin/language/' + row.id + '/edit" class="edit btn btn-success btn-sm">Edit</a>';
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.status_code === 200) {
                            ...
                            table.row(row).remove().draw();
                             ...
                             Swal.fire(
                                'Error!',
                                response.message || 'Language was not deleted.',
                                'error'
                            );
                        }
                    },
                    error: function (xhr) {
                        Swal.fire(
                            'Error!',
                            'There was an error deleting the language.',
                            'error'
                        );
                    }
                });
